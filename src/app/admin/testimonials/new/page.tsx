...
export default function CreateTestimonialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'Error creating testimonial');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto pb-16">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-[#111827]">Add Client Testimonial</h1>
        <p className="text-sm text-gray-500 mt-1">
          Add client quote, name, role, 1-5 star rating, and company logos.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. COO, Namal Education Foundation"
              className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#78B249]"
            />
          </div>
        </div>
