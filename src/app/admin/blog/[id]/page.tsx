...
export default function EditBlogPostPage({ params }: { params: { id: string } }) {
    ...
    async function loadPost() {
      try {
        ...
        if (data.success && data.post) {
          ...
          setBadge(p.badge || '');
          setCategory(p.category || 'Digital Marketing');
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto pb-16">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-[#111827]">Edit Post: {title}</h1>
        <p className="text-sm text-gray-500 mt-1">Update the post details, cover image, or content.</p>
      </div>
