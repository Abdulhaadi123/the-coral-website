...
export default function JournalPageClient({ posts }: { posts: PublicBlogPost[] }) {
  ...
  const filteredPosts =
    activeCategory === 'View All'
      ? posts
      : posts.filter((post) => post.category === activeCategory || post.badge === activeCategory);

  if (!featuredPost) {
    return (
      <main className="min-h-screen bg-white flex flex-col justify-between">
        <Header />
        <section className="max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-[13.1%] py-24 w-full text-center">
          <p className="text-gray-500">No journal posts published yet. Check back soon.</p>
        </section>
        <FooterSection />
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col justify-between">
      ...
      {/* Hero Section */}
      <section className="max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-[13.1%] pt-6 sm:pt-8 lg:pt-10 pb-8 w-full">
